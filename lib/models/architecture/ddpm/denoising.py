...
class DenoisingUnetMod(DenoisingUnet):
    ...
    def __init__(self,
        ...
        self.in_blocks = nn.ModuleList([
            EmbedSequential(
                nn.Conv2d(in_channels + concat_cond_channels, base_channels//2, 3, 1, padding=1, groups=groups))
        ])
        self.in_channels_list = [base_channels//2]

        # construct the encoder part of Unet
        for level, factor in enumerate(self.channel_factor_list):
            in_channels_ = base_channels//2 if level == 0 \
